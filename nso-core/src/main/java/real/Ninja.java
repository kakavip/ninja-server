            ...
            if (task.getTaskId() == TaskOrder.NHIEM_VU_HANG_NGAY) {
                ...
                if (task.getNvdvLevel() != 0) {
                    if (task.getNvdvLevel() >= 7) {
                        ...
                        int n = Math.max(Math.min(task.getNvdvLevel(), 9), 4) - 4;
                        ddvN += n;
                    }
                }

                int nDdv = task.getNvdvLevel() >= 5 ? util.nextInt(2) + 1 : util.nextInt(10, 13) - task.nvdvType();
                int ddvId = 695 + ddvN;
                if (ddvId > 704) {
                    ddvId = 704;
                }
                ...
                // update dv point
                this.upDVPoints(util.nextInt(5, 7), task.nvdvType());
