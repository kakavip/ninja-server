package patch;

public class Constants {
    public static final byte PK_NORMAL = 0;
    public static final byte PK_NHOM = 1;
    public static final byte PK_PHE = 1;
    public static final byte PK_BANG = 2;
    public static final byte PK_DOSAT = 3;
    public static final byte PK_TRANG = 4;
    public static final byte PK_DEN = 5;
    public static final byte PK_PHE3 = 6;

    public static final byte CHUA_VAO_LOP = 0;
    public static final byte KIEM = 1;
    public static final byte TIEU = 2;
    public static final byte KUNAI = 3;
    public static final byte CUNG = 4;
    public static final byte DAO = 5;
    public static final byte QUAT = 6;

    public static final int TRUNG_HAI_MA_ID = 601;
    public static final int TRUNG_DI_LONG_ID = 596;
    public static final byte TOC_TRUONG = (byte) 4;

    public static final int EFFECT_TANG_HINH_ID = 15;
    public static final int EFFECT_AN_THAN_ID = 16;
    public static final int EFFECT_BI_DUOC_ID = 23;

    public static final int BOSS_TU_HA_MA_THAN_ID = 223;
    public static final int BOSS_MY_HAU_TUONG_ID = 209;
    public static final int BOSS_MY_HAU_VUONG_ID = 210;
    public static final int BOSS_LAO_DAI_ID = 198;
    public static final int BOSS_LAO_TAM_ID = 199;
    public static final int BOSS_LAO_NHI_ID = 200;
}
